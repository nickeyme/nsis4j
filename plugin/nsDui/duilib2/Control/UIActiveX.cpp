...
bool CActiveXUI::DoCreateControl()
{
    ...
    if(SUCCEEDED(Hr)) {
        ...
        // (Jeffery 20140416) 修复在windows xp上获取IOleControl失败，从而导致不能显示网页。
        Hr = CoGetClassObject(CLSID_WebBrowser, CLSCTX_INPROC_SERVER | CLSCTX_INPROC_HANDLER, NULL, IID_IClassFactory,
            (void **)&pClassFactory); 
        if(SUCCEEDED(Hr) && pClassFactory) {
            if (SUCCEEDED(Hr = pClassFactory->CreateInstance(0, IID_IOleObject, (void**)&m_pUnk))) {
